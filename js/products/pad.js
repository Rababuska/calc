...
window.ProductModules['pad'] = {
    ...
    calcPrice: function(selected, config, papers, sizes, colors, laminations, toners) {
        ...
        if (layout.cover_yield === 0 || layout.back_cover_yield === 0) doesNotFit = true;

        let coverSheets = Math.ceil(selected.circulation / yCover);
        let backSheets = Math.ceil(selected.circulation / yBack);
        ...
        let coverCostPerSheet = cPaperPrice + cColorPrice + cLamPrice;
        let totalCoverCost = coverSheets * coverCostPerSheet;
        ...
        let backCostPerSheet = bcPaperPrice + bcColorPrice + bcLamPrice;
        let totalBackCost = backSheets * backCostPerSheet;
        ...
        if (selected.presentation_blocks) {
            for (let i = 0; i < selected.presentation_blocks.length; i++) {
                ...
                let bCostPerSheet = bPaperPrice + bColorPrice;
                totalBlocksCost += bSra3Sheets * bCostPerSheet;
            }
        }
