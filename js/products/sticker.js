...
window.ProductModules['sticker'] = {
    calcLayout: function(sel, config, papers, sizes, laminations) {
        ...
        let itemW = (sel.size === 'custom') ? sel.custom_width : sizes.sticker[sel.size].w;
        let itemH = (sel.size === 'custom') ? sel.custom_height : sizes.sticker[sel.size].h;
    ...
    calcPrice: function(sel, config, papers, sizes, colors, laminations) {
        ...
        let paperPrice = papers[sel.paper] ? papers[sel.paper].price : 100; // Рафлатак
        let colorPrice = CalcEngine.getColorPrice(sel.color, paperDef, false, sel, config); // Только 4+0 или 1+0
        let laminationPrice = CalcEngine.getLamPrice(sel.lamination, paperDef, false, laminations);

        let plotterCost = 0;
        if (sel.plotter_cut === 'plotter') plotterCost = config.plotter_sra3 * CalcEngine.getLenFactor(paperDef);
        if (sel.plotter_cut === 'plotter_perf') plotterCost = config.plotter_perf_sra3 * CalcEngine.getLenFactor(paperDef);
        ...
        let costPerSheet = paperPrice + colorPrice + laminationPrice + toner1Cost + toner2Cost + plotterCost;
